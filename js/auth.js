...
export const Auth = {
  ...
  validateRegister({ first, last, email, password, password2 }) {
    const errors = {};
    if (!first || !first.trim()) errors['reg-first'] = t('auth.err.enterFirstName');
    if (!last || !last.trim()) errors['reg-last'] = t('auth.err.enterLastName');
    if (!email || !isValidEmail(email)) errors['reg-email'] = t('auth.err.enterValidEmail');
    else if (!isAllowedDomain(email)) {
      errors['reg-email'] = t('auth.err.domainRestricted');
    } else if (State.users.some(u => u.email.toLowerCase() === email.toLowerCase())) {
      errors['reg-email'] = t('auth.err.emailExists');
    }
    if (!password || password.length < 6) errors['reg-password'] = t('auth.err.passwordMin');
    if (password !== password2) errors['reg-password2'] = t('auth.err.passwordMismatch');
    return errors;
  },

  validateLogin({ email, password }) {
    const errors = {};
    if (!email || !isValidEmail(email)) errors['login-email'] = t('auth.err.enterValidEmail');
    if (!password) errors['login-password'] = t('auth.err.enterPassword');
    return errors;
  },
  ...
  login({ email, password }, _skipValidation) {
    ...
    if (!user) return { ok: false, error: t('auth.err.userNotFound') };
    if (user.passwordHash !== simpleHash(password)) return { ok: false, error: t('auth.err.wrongPassword') };
