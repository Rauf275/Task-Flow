  ...
  container.querySelectorAll('.project-card').forEach(card => {
    ...
    card.addEventListener('contextmenu', (e) => {
      ...
      if (Permissions.canEditProject(State.currentUser, project)) {
        items.push({ label: '✎ Редактировать', action: () => import('./ui.js').then(m => m.openProjectModal(Projects.get(id), () => renderCurrentView())) });
        items.push({ label: '⧉ Дублировать', action: () => { Projects.duplicate(id); renderCurrentView(); } });
      }
      if (Permissions.canDeleteProject(State.currentUser, project)) {
        items.push({ label: '🗑 Удалить', danger: true, action: () => confirmDialog({
...
function populateFilterOptions(projectId) {
  ...
  assigneeSel.innerHTML = `<option value="">Исполнитель: все</option>` + members.map(m => `<option value="${m.userId}" ${m.userId === current ? 'selected' : ''}>${escapeHtml(m.user.firstName + ' ' + m.user.lastName)}</option>`).join('');
...
function renderBoard() {
  ...
  if (roleBadge) {
    roleBadge.textContent = role ? Permissions.roleLabel(role) : 'Только чтение (по ссылке)';
  ...
  board.querySelectorAll('.task-card').forEach(card => {
    ...
    card.addEventListener('contextmenu', (e) => {
      ...
      if (Permissions.canCreateTask(State.currentUser, project)) {
        menuItems.push({ label: '⧉ Дублировать', action: () => { TasksApi.duplicate(id); renderCurrentView(); } });
      }
      if (canEdit) {
        menuItems.push({ label: '🗄 В архив', action: () => { TasksApi.toggleArchive(id); renderCurrentView(); } });
      }
      if (canDelete) {
        menuItems.push({ label: '🗑 Удалить', danger: true, action: () => confirmDialog({
  ...
  board.querySelectorAll('.kanban-col-body').forEach(colBody => {
    ...
    colBody.addEventListener('drop', (e) => {
      ...
      if (!Permissions.canMoveTask(State.currentUser, project, task)) {
        toast('Недостаточно прав для перемещения этой задачи', 'error');
        return;
      }
