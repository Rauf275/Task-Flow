...
import {
  openProjectModal, openTaskModal, openProfileEditModal, openMembersModal, closeModal, confirmDialog,
} from './ui.js';
import {
...
document.getElementById('register-form').addEventListener('submit', (e) => {
  ...
  toast(t('auth.welcomeNew', { name: payload.first }), 'success');
  enterApp();
});
...
document.getElementById('login-form').addEventListener('submit', (e) => {
  ...
  toast(t('auth.welcomeBack', { name: result.user.firstName }), 'success');
  enterApp();
});
...
function enterApp() {
  ...
  if (State.currentUser) {
    const accepted = Invitations.acceptPendingFor(State.currentUser);
    if (accepted > 0) {
      toast(t('invite.acceptedToast', { n: accepted }), 'success');
    }
  }
...
document.getElementById('new-task-btn').addEventListener('click', () => {
  ...
  if (!projectId) { toast(t('toast.createProjectFirst'), 'error'); return; }
...
document.getElementById('board-project-members').addEventListener('click', () => {
  const project = Projects.get(State.currentProjectId);
  openMembersModal(project, () => renderCurrentView());
});
...
document.getElementById('bulk-delete').addEventListener('click', () => {
  ...
  confirmDialog({
    ...
    body: t('bulk.deleteConfirmBody', { n: ids.length }),
...
function checkDeadlineReminders() {
  ...
  soonTasks.forEach(t => {
    const already = State.notifications.some(n => n.title === 'notif.deadlineDue' && n.body === t.title);
    if (!already) Notifications.push('notif.deadlineDue', t.title);
  });
  renderNotifications();
}
