  ...
  create({ projectId, email, role }) {
    ...
    if (existingUser) {
      if (project.members.some(m => m.userId === existingUser.id)) {
        toast('Этот пользователь уже участвует в проекте', 'error');
        return null;
      }
      Projects.addMember(projectId, existingUser.id, role);
      toast(`${existingUser.firstName} ${existingUser.lastName} добавлен(а) в проект`, 'success');
    ...
    if (already) {
      toast('Приглашение уже отправлено на этот email', 'error');
      return null;
    }
    ...
    toast(`Приглашение отправлено на ${normalizedEmail}. Оно будет принято, как только этот email зарегистрируется в TaskFlow`, 'success');
    return invite;
  },
